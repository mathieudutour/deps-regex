...
describe("requires regex", function () {
  ...
  it("should return an array of matches", function () {
    ...
    assert.deepEqual(match('require("path")require("dirs");'), [
      "path",
      "dirs",
    ]);
    assert.deepEqual(match('var foo = require("path")require("dirs");'), [
      "path",
      "dirs",
    ]);
    assert.deepEqual(match('var\nfoo\n=\nrequire("path")require("dirs");'), [
      "path",
      "dirs",
    ]);
    assert.deepEqual(match('var foo = require("path")require("dirs")'), [
    ...
    assert.deepEqual(match('foo = require("path")require("dirs")'), [
      "path",
      "dirs",
    ]);
    assert.deepEqual(match('foo = require("path")bar = require("dirs")'), [
      "path",
      "dirs",
    ]);
    assert.deepEqual(match('foo = require("a-b")bar = require("c-d-e")'), [
      "a-b",
      "c-d-e",
    ]);
    assert.deepEqual(match('foo = require("./path")bar = require("./dirs")'), [
    ...
    assert.deepEqual(
      match('const foo = require("./path")bar = require("./dirs")'),
      ["./path", "./dirs"]
    );
  });
